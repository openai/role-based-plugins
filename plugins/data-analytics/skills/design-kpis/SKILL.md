...
Before recommending metrics or targets, determine whether the work depends on a specific required source of truth. If that source is blocked or missing, ask the user to make it available or provide an acceptable fallback. Continue with a fallback only when it is sufficient for the decision, and make the limitation visible.

Optional enrichment is different: if it is missing, continue with the strongest available evidence and note the gap when it affects confidence.

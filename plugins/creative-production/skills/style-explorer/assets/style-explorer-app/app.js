...
function storageKey() {
  return `style-explorer:${spec?.meta?.run_id || spec?.meta?.title || "default"}`;
}
...
function fallbackSvg(label = "Style anchor") {
  const svg = `
...
function renderMeta() {
  ...
  metaGrid.append(
    ...
    metaCard("Next owner", selected?.final_owner || spec.handoff?.default_owner || "generative-polish")
  );
}
...
function renderAnchor() {
  ...
  const label = selected?.label || spec.meta.anchor || "Style anchor";
  ...
  title.textContent = spec.meta.title || "Style Explorer";
  stageLabel.textContent = spec.meta.stage || "Polish exploration";
  anchorSummary.textContent = selected
    ? selected.rationale || selected.prompt || ""
    : spec.meta.summary || "Select a style system to move the anchor toward that treatment.";
  renderMeta();
}
...
function renderGrid() {
  ...
  spec.routes.forEach((route) => {
    ...
    detail.textContent = route.family || route.rationale || "Style system";
...
function buildHandoff() {
  ...
  return {
    ...
    final_owner: selected?.final_owner || spec.handoff?.default_owner || "generative-polish"
  };
}
...
function handoffMarkdown() {
  ...
  return [
    ...
    "",
    "## Preserve",
    (handoff.prompt_style_metadata.preserve || []).map((item) => `- ${item}`).join("\n") || "- None specified",
    "",
    "## Avoid",
    (handoff.prompt_style_metadata.avoid || []).map((item) => `- ${item}`).join("\n") || "- None specified",
    ""
  ].join("\n");
}
...
exportJson.addEventListener("click", () => {
  ...
  download("selected-style-route.json", content, "application/json");
  persistFeedback();
});
...
async function boot() {
  ...
  if (!spec) {
    document.body.textContent = "Missing style spec.";
    return;
  }

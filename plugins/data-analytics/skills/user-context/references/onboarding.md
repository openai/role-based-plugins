...
```md
**Onboarding Complete**
You're set up to use Data Analytics with configured source routes or explicit fallbacks, a reusable semantic-layer path, optional weekly polling if you enabled it, and a first analysis workflow ready to reuse.
...
1. Orientation
2. Confirm active/missing sources
3. Semantic layer setup
4. Hero prompt

`Semantic layer refresh setup` is a substep of semantic-layer setup, not a separate top-level onboarding milestone. Keep it in onboarding state because it matters operationally, but do not make the top-level experience feel like a fourth major setup track before the user sees value.
...
2. Set up one reusable semantic layer for a real product or business area
...
**Next Step**
Reply `continue` and I'll check the main source categories one by one, only asking where you need to choose, connect, or use a fallback.
```
...
- User-facing goal: Give the user confidence about which source categories look usable and ask only for help where a source is ambiguous, missing, or needs IT or admin enablement.
...
Core setup still needs your choice before Data Analytics can set up a reusable semantic layer:

1. **Data warehouse or SQL** validates metric definitions, table shape, query logic, and source-backed answers from current data.
   Options: Databricks, BigQuery, Snowflake.
2. **Team communication** adds recent discussion, owners, caveats, and decision context behind metric movement or source choices.
   Options: Slack, Teams.

Company docs look covered through Google Drive and Notion.

There are a few optional connectors we can add later when a workflow or semantic-layer plan needs them:

3. **Dashboards or BI, if you use them** adds report definitions, filters, saved views, and executive-ready dashboard context.
...
5. **Notebook lab, if you use it** adds prior analyses, notebook logic, and reusable exploratory context.
   Options: Hex, Deepnote.

You'll be prompted to install and authenticate plugins or connectors for whichever you select.

**Next Step**
Reply with the core sources you want to set up now, or say `defer core sources` if you want to continue with weaker analytics from manual or provided context.
```
...
```md
Please select the core sources you want to set up now:
...
You can defer a core source, but say it explicitly so Data Analytics records the known gap before moving on. Optional sources can wait; Data Analytics will ask again later only when a workflow would materially benefit from one of them.
```
...
Before we run the first analysis, let's make one analytical area reusable. A semantic layer gives Data Analytics a saved home for the metric definitions, source choices, joins, caveats, and query patterns behind one real product or business area, so future answers do not start cold.

Send one area to make reusable first, plus at least one seed source. Even one is enough.

Examples:

- `subscription ARR`, seeded by Databricks table `example.analytics.fact_subscription_arr`
- `paid workspace retention`, seeded by this dashboard link: `...`
- `customer onboarding funnel`, seeded by this SQL query: `...`
- `weekly revenue reporting`, seeded by the metrics doc and `#team-channel`
```
...
Semantic-layer setup is the second major onboarding milestone. The refresh automation offer exists, but it should feel like a small follow-up after the layer is real, not a separate onboarding detour before the user sees value.

### Step 3A: Introduce Semantic Layer Setup

- ID: `introduce_semantic_layer_setup`
- Type: `linear`
- Parent: `semantic_layer_setup`
- State field: `semantic_layer_setup`
- User-facing goal: Explain what a semantic layer is and ask for the smallest real anchor needed to create one well.
...
- Stay here when: The semantic-layer concept has not been introduced or the user has not provided enough intake to create, plan, skip, defer, or block the layer.
- Exit when: The user provides one coherent product or business area plus enough seed context to proceed, asks for intake-only planning, skips, defers, or is blocked on one concrete missing input.
- Completion: Record the selected area or skip/defer/block resolution in `onboarding-state.json`.
- Next: Step 3B, `create_or_plan_semantic_layer`.
- Output contract: Explain that a semantic layer makes metric definitions, source choices, joins, caveats, and query patterns reusable for one product or business area. Ask for one product or business area plus any useful seed source such as a dashboard, table, SQL query, doc, repo path, notebook, team thread, or recurring business question. Keep the ask concrete and reply-shaped:

```md
## Semantic Layer Setup

A semantic layer is reusable context for one product or business area: metric definitions, source choices, joins, caveats, and query patterns that future Data Analytics runs can load before choosing tables or writing SQL.
...
- Do not: merge unrelated product areas into one layer unless the user explicitly asks, ask for a broad source inventory before the user has named the area, or pretend an unseeded skeleton is source-backed.
...
- Entry condition: The user supplied one coherent area and enough seed context to proceed, or explicitly requested intake-only planning.
...
- Completion: Write durable semantic-layer pointers to `user-context.md`; write operational setup status, source inventory path, seed source count when useful, and compact connector-gap labels to `onboarding-state.json`. Do not copy seed source URLs, source inventory rows, or crawl output into onboarding state.
...
- Output contract: Read `semantic-layer/setup.md`, `semantic-layer/source-intake.md`, and `semantic-layer/connector-playbook.md`. Resolve connectors lazily as the selected evidence lanes need them. Build the source inventory before crawling. If a needed source route is selected but has not yet been read successfully in the current workflow, attempt the actual source read only when that semantic-layer lane needs it, then handle auth, query, schema, skip, or manual fallback in context. When enough source-backed context exists, create or refresh the target standalone semantic-layer skill, validate it, write the durable pointer into `user-context.md`, and report the exact created or updated paths plus connector gaps. If blocked, state the exact missing connector, permission, source, or destination decision and ask for the smallest useful fallback.
...
The hero prompt is the first value demonstration. Show one strong prompt first, and make it report-first, built from real context gathered during source setup confirmation and semantic-layer setup. Do not start with a three-option chooser. After the first prompt is run, skipped, or deferred, finish onboarding. Offer more prompt ideas only when the user explicitly asks for them later.
...
- Completion: Use `complete` when source setup confirmation, semantic-layer setup plus refresh substep resolution, and first hero prompt resolution are done. Use `quiet` when the user does not want more onboarding nudges.
- Terminal frame:
...
**Onboarding Complete**
You're set up to use Data Analytics with configured source routes or explicit fallbacks, a reusable semantic-layer path, and a first analysis workflow ready to reuse. Start a new thread with any real analytics question, or keep going with another suggested prompt when useful.
```
...
1. Source setup confirmation has covered each core source category as `active`, or as a user-explicit `skipped`, `deferred`, `declined`, `unavailable`, or `not_applicable` fallback recorded with `resolution: user_skipped`, `resolution: user_deferred`, `resolution: user_declined`, `resolution: user_marked_unavailable`, `resolution: user_confirmed_not_applicable`, or `resolution: user_continued_with_known_gap`. A manual source route counts as `active` only when its state entry records `source_kind: manual`; `needs_confirmation`, `missing`, or a core fallback without one of those explicit resolution values still require the user to choose, install, skip, defer, decline, mark unavailable, mark not applicable, or continue with a recorded known gap before core setup is complete.
2. One semantic layer is created, refreshed, inspected, repaired, planned with a stable target path and source inventory, skipped, deferred, or blocked on one concrete missing input. The semantic-layer refresh substep is accepted and read back, declined, deferred, unavailable, skipped, or blocked on one concrete prerequisite.
...
This is required before Data Analytics can reliably use your configured sources and authoritative semantic-layer context. It also gives you a useful overview of the plugin's key functionality.
...
Use this only after an ordinary Data Analytics workflow when onboarding is already active and a reminder would be useful. Do not use the `## Data Analytics Setup Required` start CTA, do not ask whether to start onboarding, and do not compete with the skill's final action close. Keep the note short and non-final, for example:

```md
Data Analytics onboarding is still in progress; when you want to resume it, say `continue onboarding` in this thread.
```
